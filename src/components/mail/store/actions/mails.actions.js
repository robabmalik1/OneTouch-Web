...
import {deleteMessage, getMessageList} from "./messageListActionCreators";
...
export function updateMails()
{
    return (dispatch, getState) => {
        ...
        return dispatch(getMessageList("INBOX"))
        // const {routeParams} = getState().mailApp.mails;
        //
        // const request = axios.get('Mail/inbox', {
        //     params: routeParams
        // });
        //
...
export function setFolderOnSelectedMails(id)
{
    return (dispatch, getState) => {
        ...
        selectedMailIds.map((id,index)=>{
            dispatch(deleteMessage(id));
        }
        )
        setTimeout(()=>{dispatch(getMessageList("INBOX"));},3000)
